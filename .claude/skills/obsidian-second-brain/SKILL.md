...
description: Use when reading/writing Obsidian vault notes, creating daily notes, capturing decisions, or mirroring Docker memory to Obsidian. Trigger phrases: write to Obsidian, daily note, vault, decision log, sync to Obsidian, mirror, search my notes.
...
Obsidian and Docker memory serve different purposes and must never duplicate each other. Docker is always primary for session rehydration; Obsidian holds long-form human-readable content.

For full mirroring patterns (Mirror a Docker Decision, Link a Session Note Back to Docker, content storage matrix), see [reference.md](reference.md).
