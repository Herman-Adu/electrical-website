/**
 * TDD: CategoriesCircuit tests
 * Covers:
 *   1. Renders without crashing
 *   2. SVG has correct viewBox
 ...
 *   5. Open circles render
 *   6. Vertical accent lines render
 *   7. Scanline is present
 */
...
import { CategoriesCircuit } from "../categories-circuit";

// ---------------------------------------------------------------------------
// Tests
// ---------------------------------------------------------------------------
describe("CategoriesCircuit", () => {
  describe("Rendering", () => {
    it("renders without crashing", () => {
      const { container } = render(<CategoriesCircuit shouldReduceMotion={false} />);
    ...
    it("SVG has the correct viewBox", () => {
      const { container } = render(<CategoriesCircuit shouldReduceMotion={false} />);
      const svg = container.querySelector("svg");
      expect(svg?.getAttribute("viewBox")).toBe("0 0 1440 700");
    });
    ...
    it("renders 3 vertical accent lines", () => {
      const { container } = render(<CategoriesCircuit shouldReduceMotion={false} />);
      const lines = container.querySelectorAll("line");
      expect(lines.length).toBe(3);
    });

    it("renders open circles (stroke only)", () => {
      const { container } = render(<CategoriesCircuit shouldReduceMotion={false} />);
  ...
  describe("shouldReduceMotion=true — no animation", () => {
    it("paths have duration 0 in transition when shouldReduceMotion=true", () => {
      const { container } = render(<CategoriesCircuit shouldReduceMotion={true} />);
