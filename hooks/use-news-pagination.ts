...
interface UseNewsPaginationOptions<T> {
...
interface UseNewsPaginationReturn<T> {
...
/**
 * Hook for managing paginated list state without race conditions.
 * Uses React 19's useTransition for proper loading semantics.
 */
export function useNewsPagination<T>({
  items,
  initialCount = 4,
  batchSize = 3,
}: UseNewsPaginationOptions<T>): UseNewsPaginationReturn<T> {
