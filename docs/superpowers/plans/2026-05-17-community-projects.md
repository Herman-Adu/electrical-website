# Community Projects Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add two `Project` entries to the `community` category — Thames Valley Adventure Playground (PAT testing, planned) and Slough in Bloom (community sponsorship, in-progress).

**Architecture:** Dedicated `data/projects/community.ts` exports `communityProjects: Project[]`. `data/projects/index.ts` imports and spreads it into `allProjects`. No route, component, or type changes required — both projects are served automatically at `/projects/category/community/[slug]`.

**Tech Stack:** TypeScript (strict), Next.js 16, `@/types/projects`, `@/types/sections`
...
|--------|------|----------------|
| Create | `data/projects/community.ts` | Exports `communityProjects: Project[]` — TVAP + Slough in Bloom entries |
| Modify | `data/projects/index.ts` line 1–10 (imports) | Add `import { communityProjects } from "./community"` |
| Modify | `data/projects/index.ts` line 61 (`allProjects`) | Spread `...communityProjects` as first items |

---

## Task 1: Create `data/projects/community.ts`

**Files:**
- Create: `data/projects/community.ts`
...
Expected: no errors referencing `data/projects/community.ts`. If TypeScript reports an icon name as invalid, cross-check against the `ProjectScope.icon` union in `types/projects.ts` (lines 78–97) and the `TrustIndicatorItem.icon` union in `types/sections.ts`.

---

## Task 2: Wire community projects into `allProjects`

**Files:**
- Modify: `data/projects/index.ts` (add import at top, spread into `allProjects`)

- [ ] **Step 1: Add the import**

In `data/projects/index.ts`, after the existing `import type { ... } from "@/types/projects"` block (around line 8), add:
...
- [ ] **Step 2: Spread community projects into `allProjects`**
...
Expected: typecheck passes, build completes with no errors. If the build reports a missing image (404-style warning), verify the file exists at the exact path listed in the `coverImage.src` or `gallery[*].src` fields. All paths use `/images/...` (no `/public` prefix).

- [ ] **Step 2: Commit**

```bash
git add data/projects/community.ts data/projects/index.ts
git commit -m "feat(community): add TVAP + Slough in Bloom project entries

- data/projects/community.ts: new file, exports communityProjects[]
- TVAP: planned status, pro bono PAT testing, 4-image gallery
- Slough in Bloom: in-progress, 2023+2026 sponsorship, 13-image gallery
- data/projects/index.ts: import + spread communityProjects into allProjects
...
**Spec coverage check:**
- [x] TVAP card fields (id, slug, category, status, progress, heroHeadline, heroIndicators, coverImage, kpis, tags) — all present
- [x] TVAP detail.intro with headlineWords, leadParagraph, bodyParagraphs, pillars — complete
- [x] TVAP narrativeBlock after-intro "What Community Means to Nexgen" — present
- [x] TVAP scope 6 items (Wrench/Zap/Shield/Award/Settings/Activity) — all valid ProjectScope icons
- [x] TVAP timeline 3 phases all `upcoming` — matches planned status
- [x] TVAP gallery 4 images with renamed filenames — matches filesystem
- [x] Slough card fields — complete
- [x] Slough detail.intro with comeback narrative — complete
- [x] Slough narrativeBlock after-intro "Same Values. New Name." — present
- [x] Slough narrativeBlock after-challenge "Why Local Business Shows Up" — present
- [x] Slough scope 6 items (Award/Network/Lightbulb/CheckCircle/Building/Activity) — all valid ProjectScope icons
- [x] Slough challenge + solution strings — present
- [x] Slough timeline 3 phases (2 completed, 1 in-progress) — present
- [x] Slough gallery 13 images with renamed filenames — matches filesystem
- [x] Slough testimonial (Bruce Hicks) — present
- [x] Intact Electrical never named — confirmed
- [x] `description` field (required by Project interface) — added to both entries
- [x] `heroIndicators as const` — present on both entries
- [x] `allProjects` spread — Task 2

**Type consistency:** All icon names used match the spec's validated icon lists. `ProjectScope.icon` uses `Award|Network|Lightbulb|CheckCircle|Building|Activity|Wrench|Zap|Shield|Settings`. `TrustIndicatorItem.icon` uses `ClipboardCheck|Heart|Users|MapPin|Star|Lightbulb`.
